...
사용자 지시에 따라 국내 장종료 상태를 우회해 KIS paper 해외주식/미국장 경로를 진행했다. 공식 KIS 샘플에서 확인된 해외주식 주문/정정취소/주문체결조회/잔고조회 endpoint와 paper TR ID만 사용했고, 모든 gate는 현재 Python 검증 프로세스에만 주입했다. `.env`/`.env.local`은 수정하지 않았고, live 주문, live cancel, live fallback, unattended WebSocket loop, raw secret 출력은 수행하지 않았다.

| 항목 | 결과 | 근거 |
|---|---|---|
| 해외주식 adapter 구현 | 완료 | `venue=NASD/NYSE/AMEX` 또는 `PAPER_TRADING_MARKET=US`일 때 `/uapi/overseas-stock/v1/trading/order`, `/order-rvsecncl`, `/inquire-ccnl`, `/inquire-balance`로 분기. 미국 paper 주문 TR은 `VTTT1002U`/`VTTT1006U`, 취소 `VTTT1004U`, 체결조회 `VTTS3035R`, 잔고조회 `VTTS3012R`만 사용 |
| API/bot submit metadata | 완료 | `PaperConfigService`가 `PAPER_TRADING_MARKET`, `KIS_OVERSEAS_EXCHANGE_CODE`, `KIS_OVERSEAS_CURRENCY`를 런타임 config로 노출하고, `PaperOrderService` network submit이 `market=US`, `venue/exchange=NASD`, `currency=USD` metadata를 adapter에 전달 |
...
| US regular-session guard | 완료 | Phase 12C/21 helper는 `market=US`일 때 `America/New_York` 정적 정규장 `09:30-16:00`과 평일 조건을 확인하고, 조건 불충족 시 adapter 호출 전 `US_REGULAR_SESSION_REQUIRED`로 중단. 휴장일 calendar feed는 아직 사용하지 않음 |
| Phase 21 activation helper | 완료 | `tools/kis_paper_phase21_us_activation.py` 추가. token 발급, WebSocket approval, subscription preview, bounded smoke, controlled US submit/list/sync/cancel을 한 redacted record로 묶고 각 단계는 별도 confirmation token 없이는 network를 열지 않음 |
...
| 2026-05-28 05:13 New York retry request | 정규장 전 차단 | `.env.local`을 현재 PowerShell 프로세스에만 로드한 뒤 `tools\kis_paper_phase21_us_activation.py --execute-submit --symbol AAPL --exchange NASD --currency USD` 실행. preflight는 통과했지만 `trading_window_closed`, `US_REGULAR_SESSION_REQUIRED`, `network_call_performed=false`, adapter/list/sync/cancel 호출 없음 |
| Redacted records | 생성 | `docs/research/kis-paper-phase21-us-redacted-record.json`, `docs/research/kis-paper-phase21-us-window-guard-redacted-record.json` |
...
| US window guard regression | 통과 | `.\.venv\Scripts\python.exe -m pytest backend/tests/test_kis_paper_phase21_us_activation_tool.py backend/tests/test_kis_paper_phase12c_tool.py -q -p no:cacheprovider --basetemp $env:TEMP\stock_phase21_us_window_gate` -> `15 passed in 0.14s` |
...
| Paper/no-live regression after US window guard | 통과 | `.\.venv\Scripts\python.exe -m pytest backend/tests/test_kis_paper_phase21_us_activation_tool.py backend/tests/test_kis_paper_phase12c_tool.py backend/tests/test_kis_paper_token_websocket_activation.py backend/tests/test_kis_paper_adapter_contract.py backend/tests/test_kis_token_lifecycle_phase1.py backend/tests/test_kis_token_manager.py backend/tests/test_paper_order_service.py backend/tests/test_paper_order_api.py backend/tests/test_paper_sync_service.py backend/tests/test_paper_realtime_worker.py backend/tests/test_paper_dashboard_report_phase6.py backend/tests/test_api_smoke.py backend/tests/test_phase3d_broker_safety.py backend/tests/test_phase3e_paper_safety.py backend/tests/test_no_live_adapter.py backend/tests/test_no_live_trading_regression.py backend/tests/test_final_safety_hardening.py -q -p no:cacheprovider --basetemp $env:TEMP\stock_phase21_us_window_regression` -> `71 passed in 35.67s` |
...
결론: 미국장 경로에서 token 발급, WebSocket approval, 실제 paper WebSocket 연결/구독 ACK, 해외 price/balance 조회, 해외주식 주문 endpoint 도달까지 확인했다. API/bot service submit 경로도 미국장 runtime metadata를 adapter까지 전달하도록 보강했고, broker order id가 생성된 뒤 sync fill/position이 기존 paper order에 귀속되는 persistence 경로를 확인했다. controlled helper도 미국장 metadata로 kill-switch proof -> submit -> list -> sync -> cancel을 수행할 수 있게 확장했으며, Phase 21 helper는 token/WebSocket/submit lifecycle을 한 redacted record로 묶는다. KIS가 주문 submit을 HTTP 500으로 거부했고 이후 read-only sync도 rate-limit로 중단되어 실제 broker 주문 생성, 체결, 포지션 변경은 아직 완료되지 않았다. 최신 요청은 미국장 기준으로 진행했지만 현재 뉴욕 정규장 전이어서 `US_REGULAR_SESSION_REQUIRED`로 submit adapter 호출 전 차단했다. 다음 재시도는 미국 정규장 `09:30-16:00 America/New_York` 안에서 별도 조건 확인 후 1회만 수행하고, 거부/auth/rate-limit/stale 응답이면 재시도하지 않는다.
