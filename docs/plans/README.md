...
이 디렉터리는 단계별 개발 계획과 승인 경계를 관리한다. 현재 기준선은 `MVP v0.24.0 / KIS Paper Broker Phase 2 KIS Paper Broker Contract`이다.
...
| 항목 | 값 |
|---|---|
| 현재 checkpoint | `KIS Paper Broker Phase 2 KIS Paper Broker Contract` |
| 현재 구현 완료 | Weekly Review Report, Trade Ledger Foundation, Portfolio Risk Guard v2, breadth/Data Reliability 2, Validation Framework Scaffold, Parameter Snapshot Foundation, Walk-forward/PBO/DSR minimal validation, Factor/Filter Attribution, KIS paper broker Phase 0 audit, Phase 1 notification foundation, Phase 2 broker contract |
| 최신 backend pytest | Phase 2 targeted/regression `22 passed`, full backend `293 passed` |
...
1. KIS paper broker Phase 3 Paper Trading Persistence는 additive-only migration으로 진행한다.
