...
이 프로젝트는 단일 프로그램이다. 하나의 FastAPI 프로세스가 Next.js static export를 같은 origin에서 서빙하며, pywebview 네이티브 창으로 감싼 단일 Windows `.exe`로 패키징한다. 별도의 frontend 프로세스(`next start`, `:3000`)나 별도 backend를 가리키는 `NEXT_PUBLIC_API_BASE_URL`은 없다. UI는 한국어이며 monospace + beige 라이트 테마에 라이트/다크 토글 슬라이더를 제공한다. 로그인은 로컬 JSON 기반(`backend/data/users.json`, PBKDF2-HMAC-SHA256, HMAC 30일 토큰)이며 사용자가 한 명이라도 생기면 인증이 강제되는 opt-in 방식이다.
...
운영 형태는 단일 프로세스다. 하나의 FastAPI 프로세스가 Next.js static export를 같은 origin에서 서빙하며, `launcher.py`가 `app_main.py --host 127.0.0.1 --port 8000` 단일 프로세스를 띄우고 pywebview 네이티브 창으로 감싼다. 별도 frontend 프로세스(`next start`, `:3000`)는 띄우지 않는다. frontend는 같은 origin이므로 `NEXT_PUBLIC_API_BASE_URL`은 비워 둔다(same-origin).

먼저 frontend static export를 빌드한 뒤 backend가 이를 서빙한다. Windows에서는 `npm` 정책 오류를 피하기 위해 `npm.cmd`를 우선 사용한다.
...
backend(static export 서빙)를 직접 실행할 때는 프로젝트 루트에서 다음을 사용한다.
...
- 실계좌 KIS live 주문은 활성화되어 있으나(사용자 본인 계좌, 고위험), 기본은 fail-closed다. `LIVE_TRADING_ENABLED` + `LIVE_ORDER_SUBMIT_ENABLED` + `ENABLE_REAL_ORDER` + live 자격증명 + live host + per-order 확인 + kill switch + max-notional 게이트를 모두 통과해야만 제출된다. KRX 국내 현금 주문만 허용한다. 이 게이트를 약화하거나 우회하지 않는다. live 실행 경로는 실제 KIS API 대비 아직 미검증이므로 최초 주문은 최소 수량으로 검증한다.
...
- `/api/settings`는 `backend/config/*.yaml`을 read-only로 보여주며 시크릿 raw 값을 반환하지 않는다.
- 토글 가능한 런타임 설정은 UI에서 변경하면 `backend/data/runtime_env.json`에 영속화된다(allowlist 키만, 값은 마스킹). 현재 프로세스에만 반영되는 process-only 방식이 아니며, 자격증명도 앱 안에서 입력할 수 있다.

## 완료 기준

- 요청된 범위가 실제 파일에 반영되어야 한다.
- 공개 API, DB schema, 런타임 로직을 바꿨다면 관련 테스트와 빌드를 통과해야 한다.
- broker/paper 관련 작업은 실행 모드가 `analysis_only`, `telegram_report`, `paper_kis`, live 중 어디에 해당하는지 확인해야 한다.
- `paper_kis` 작업은 `paper_orders`/`paper_fills`/`paper_positions`와 audit log 기준으로 검증한다. live 경로는 fail-closed 게이트(`LIVE_TRADING_ENABLED` + `LIVE_ORDER_SUBMIT_ENABLED` + `ENABLE_REAL_ORDER` + live 자격증명/host + per-order 확인 + kill switch + max-notional)가 의도대로 동작하는지 확인하고, KRX 현금 주문만 허용되는지 검증한다.
...
```powershell
rg -n "Repo Layout|단일 프로세스 실행 방법|Test / Lint / Build|Coding Conventions|절대 하지 말아야 할 것|완료 기준|검증 방법" AGENTS.md
```
...
- frontend는 backend와 same-origin이므로 별도 API base URL 설정 없이 접근되는지 확인
