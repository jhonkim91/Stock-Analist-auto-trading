...
- 최신 재확인 기준 `KIS_REFRESH_TOKEN`은 Process/User/Machine scope와 프로젝트 루트 `.env.local` 모두에서 미탐지다. `.env.local`에는 `KIS_ACCESS_TOKEN`이 있어 `access_token_without_refresh_token=true`로 진단되며, 해당 access token은 `2026-05-28T12:15:08+00:00` 기준 만료 상태라 refresh token proof를 대체할 수 없다.
...
- [x] `.\.venv\Scripts\python.exe tools\kis_live_token_refresh_preflight.py --load-env-local` -> `.env.local` no-network 로드, `access_token_without_refresh_token=true`, `access_token_expired=true`, `refresh_token_configured=false`, `network_call_performed=false`, `live_order_created=false`.
- [x] `.\.venv\Scripts\python.exe tools\live_phase3_completion_audit.py --load-env-local --write-record` -> `complete=false`, `access_token_env_file_configured=true`, `access_token_expired=true`, `KIS_REFRESH_TOKEN` 미탐지, token refresh proof/live submit authority 미충족.
