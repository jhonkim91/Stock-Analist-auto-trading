...
- [x] 현재 상태명: `Phase C-3 new_high_breakout Strategy`
...
- Strategy default: `trend_breakout`, `vcp_breakout`, `canslim_lite`, `new_high_breakout` 4개 기본 실행.
- Strategy available: 기본 4개에 더해 `momentum_rank`, `relative_strength_leader`는 명시 선택 시 screener/backtest에서 실행 가능.
- `new_high_breakout`: `high_52w`, `distance_from_52w_high`, `breakout`, `volume`, `volume_ma50`, `volume_ratio_50`, `rs_percentile`, 이동평균 추세를 사용하며 DB schema 변경이 없다.
...
- `backend/app/strategies/new_high_breakout.py`: 52주 신고가/근접 돌파 전략 추가.
- `backend/app/strategies/registry.py`: `new_high_breakout`을 default와 available registry에 등록.
- `backend/config/strategies.yaml`: `new_high_breakout` 기본 threshold 추가.
- `backend/app/services/screener_service.py`: `new_high_breakout` Screener 응답에 strategy별 `data_quality_flags` 병합.
- `backend/tests/test_strategies.py`: C-3 pass/fail fixture, registry, Screener/Backtest 실행 테스트 추가.
- `README.md`, `docs/VALIDATION.md`, `docs/PROJECT_STATUS.md`, `Memory.md`: Phase C-3 기준으로 최신 상태 갱신.
...
- 2026-05-23 `.\.venv\Scripts\python.exe -m pytest backend/tests/test_strategies.py backend/tests/test_backtest.py -q`: 44 passed.
- 2026-05-23 `.\.venv\Scripts\python.exe -m pytest backend/tests`: 122 passed.
...
- [ ] `new_high_breakout`, `relative_strength_leader`를 frontend strategy selector에 추가할지 별도 범위로 결정한다.
